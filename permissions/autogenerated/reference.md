## Default Permission

Default permissions for the media-parser plugin

#### This default permission set includes the following:

- `allow-get-metadata`
- `allow-get-tracks`
- `allow-get-subtitles`
- `allow-get-thumbnails`

## Permission Table

<table>
<tr>
<th>Identifier</th>
<th>Description</th>
</tr>


<tr>
<td>

`media-parser:allow-get-metadata`

</td>
<td>

Enables the get_metadata command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`media-parser:deny-get-metadata`

</td>
<td>

Denies the get_metadata command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`media-parser:allow-get-subtitles`

</td>
<td>

Enables the get_subtitles command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`media-parser:deny-get-subtitles`

</td>
<td>

Denies the get_subtitles command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`media-parser:allow-get-thumbnails`

</td>
<td>

Enables the get_thumbnails command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`media-parser:deny-get-thumbnails`

</td>
<td>

Denies the get_thumbnails command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`media-parser:allow-get-tracks`

</td>
<td>

Enables the get_tracks command without any pre-configured scope.

</td>
</tr>

<tr>
<td>

`media-parser:deny-get-tracks`

</td>
<td>

Denies the get_tracks command without any pre-configured scope.

</td>
</tr>
</table>
