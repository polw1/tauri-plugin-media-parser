...
use crate::decoders::h264::frame::Crop;
...
pub(super) fn crop_from_aperture(
   ...
   if x % 2 != 0 || y % 2 != 0 {
      return Err(unsupported("display aperture offsets must be even"));
   }
   if x.checked_add(width).is_none_or(|end| end > coded_width)
      || y.checked_add(height).is_none_or(|end| end > coded_height)
   {
      return Err(unsupported("display aperture exceeds coded geometry"));
   }
   Ok(Crop {
      x,
      y,
