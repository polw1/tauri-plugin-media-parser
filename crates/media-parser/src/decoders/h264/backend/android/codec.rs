...
use ndk_sys::{
   ...
   AMediaFormat_new, AMediaFormat_setBuffer, AMediaFormat_setInt32, AMediaFormat_setString,
   media_status_t,
};
...
impl H264Decoder for AndroidDecoder {
   fn open(config: &AvcConfig) -> Result<Self, DecodeError> {
      ...
      let codec = unsafe { AMediaCodec_createDecoderByType(MIME_AVC.as_ptr().cast()) };
